	...
	ValueListNode* order, RowsClause* rows, bool updateLock, USHORT flags)
{
	...
	const bool sortWindow = rse->dsqlOrder &&
		AggregateFinder::find(dsqlScratch->getPool(), dsqlScratch, true, rse->dsqlOrder);

	// WINDOW functions
	if ((rse->dsqlSelectList &&
			AggregateFinder::find(dsqlScratch->getPool(), dsqlScratch, true, rse->dsqlSelectList)) ||
		sortWindow)
	{
		...
		for (FB_SIZE_T i = 0, mapCount = parent_context->ctx_win_maps.getCount(); i < mapCount; ++i)
		{
			...
			if (partitionMap->partition)
			{
				...
				FieldRemapper remapper2(dsqlScratch->getPool(), dsqlScratch, parent_context, true,
					partitionMap->partition, partitionMap->order);
