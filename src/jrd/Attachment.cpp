...
Profiler* Attachment::getProfiler(thread_db* tdbb)
{
	auto profiler = att_profiler.get();
	if (!profiler)
		att_profiler.reset(profiler = Profiler::create(tdbb));
	return profiler;
}

bool Attachment::isProfilerActive()
{
	return att_profiler && att_profiler->isActive();
}
